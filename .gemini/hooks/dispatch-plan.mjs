#!/usr/bin/env node
// Gemini CLI BeforeAgent hook → shared role-router task classifier.
// Injects the dispatch plan (task type, suggested role chain, parallel/sequential)
// into CO's context for every user prompt. Fails open on any error.
...
try {
    ...
    process.stdout.write(JSON.stringify({
        hookSpecificOutput: { additionalContext: formatDispatchContext(plan) },
    }));
    process.exit(0);
} catch {
